///Register `HCBULKCURRENTED` reader
...
///Register `HCBULKCURRENTED` writer
...
///Field `BCED` reader - BulkCurrentED This is advanced to the next ED after the HC has served the current one.
...
///Field `BCED` writer - BulkCurrentED This is advanced to the next ED after the HC has served the current one.
pub struct BCED_W<'a> {
    w: &'a mut W,
}
impl<'a> BCED_W<'a> {
...
///`read()` method returns [hcbulkcurrented::R](R) reader structure
impl crate::Readable for HCBULKCURRENTED_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [hcbulkcurrented::W](W) writer structure
impl crate::Writable for HCBULKCURRENTED_SPEC {
    type Writer = W;
}
///`reset()` method sets HCBULKCURRENTED to value 0
