///Register `DATA_PAYLOAD_BASE_ADDR` reader
...
impl From<crate::R<DATA_PAYLOAD_BASE_ADDR_SPEC>> for R {
    #[inline(always)]
    fn from(reader: crate::R<DATA_PAYLOAD_BASE_ADDR_SPEC>) -> Self {
        R(reader)
    }
}
///Register `DATA_PAYLOAD_BASE_ADDR` writer
...
impl From<crate::W<DATA_PAYLOAD_BASE_ADDR_SPEC>> for W {
    #[inline(always)]
    fn from(writer: crate::W<DATA_PAYLOAD_BASE_ADDR_SPEC>) -> Self {
        W(writer)
    }
}
///Field `DAT_BASE` reader - Base address to be used by the hardware to find the start of the data payload section.
...
///Field `DAT_BASE` writer - Base address to be used by the hardware to find the start of the data payload section.
...
///`read()` method returns [data_payload_base_addr::R](R) reader structure
impl crate::Readable for DATA_PAYLOAD_BASE_ADDR_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [data_payload_base_addr::W](W) writer structure
impl crate::Writable for DATA_PAYLOAD_BASE_ADDR_SPEC {
    type Writer = W;
}
///`reset()` method sets DATA_PAYLOAD_BASE_ADDR to value 0
