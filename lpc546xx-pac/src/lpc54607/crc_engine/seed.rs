...
///Field `CRC_SEED` reader - A write access to this register will load CRC seed value to CRC_SUM register with selected bit order and 1's complement pre-processes. A write access to this register will overrule the CRC calculation in progresses.
...
///Field `CRC_SEED` writer - A write access to this register will load CRC seed value to CRC_SUM register with selected bit order and 1's complement pre-processes. A write access to this register will overrule the CRC calculation in progresses.
...
///`read()` method returns [seed::R](R) reader structure
impl crate::Readable for SEED_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [seed::W](W) writer structure
impl crate::Writable for SEED_SPEC {
    type Writer = W;
}
///`reset()` method sets SEED to value 0xffff
