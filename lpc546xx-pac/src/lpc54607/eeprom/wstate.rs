...
///Field `PHASE3` reader - Wait states for phase 3 (minus 1 encoded).
...
///Field `PHASE3` writer - Wait states for phase 3 (minus 1 encoded).
...
///Field `PHASE2` reader - Wait states for phase 2 (minus 1 encoded).
...
///Field `PHASE2` writer - Wait states for phase 2 (minus 1 encoded).
...
///Field `PHASE1` reader - Wait states for phase 1 (minus 1 encoded).
...
///Field `PHASE1` writer - Wait states for phase 1 (minus 1 encoded).
...
///Field `LCK_PARWEP` reader - Lock timing parameters for write, erase and program operation 0 = WSTATE and CLKDIV registers have R/W access 1 = WSTATE and CLKDIV registers have R only access.
...
///Field `LCK_PARWEP` writer - Lock timing parameters for write, erase and program operation 0 = WSTATE and CLKDIV registers have R/W access 1 = WSTATE and CLKDIV registers have R only access.
...
///`read()` method returns [wstate::R](R) reader structure
impl crate::Readable for WSTATE_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [wstate::W](W) writer structure
impl crate::Writable for WSTATE_SPEC {
    type Writer = W;
}
///`reset()` method sets WSTATE to value 0x0004_0802
