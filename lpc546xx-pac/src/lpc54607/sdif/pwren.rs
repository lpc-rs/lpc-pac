...
///Field `POWER_ENABLE` reader - Power on/off switch for card; once power is turned on, software should wait for regulator/switch ramp-up time before trying to initialize card.
...
///Field `POWER_ENABLE` writer - Power on/off switch for card; once power is turned on, software should wait for regulator/switch ramp-up time before trying to initialize card.
...
///`read()` method returns [pwren::R](R) reader structure
impl crate::Readable for PWREN_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [pwren::W](W) writer structure
impl crate::Writable for PWREN_SPEC {
    type Writer = W;
}
///`reset()` method sets PWREN to value 0
