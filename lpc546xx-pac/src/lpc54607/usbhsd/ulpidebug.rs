...
///Field `PHY_ADDR` reader - ULPI mode: Bits 7:0 are used as the address when doing a register access over the ULPI interface.
...
///Field `PHY_ADDR` writer - ULPI mode: Bits 7:0 are used as the address when doing a register access over the ULPI interface.
...
///Field `PHY_RDATA` reader - UTMI+ mode: Bits 7:0 contains the value returned by the VStatus signal on Vendor Interface of UTMI+.
...
///Field `PHY_RDATA` writer - UTMI+ mode: Bits 7:0 contains the value returned by the VStatus signal on Vendor Interface of UTMI+.
...
///Field `PHY_MODE` reader - This bit indicates if the interface between the controller is UTMI+ or ULPI.
...
///Field `PHY_MODE` writer - This bit indicates if the interface between the controller is UTMI+ or ULPI.
...
///`read()` method returns [ulpidebug::R](R) reader structure
impl crate::Readable for ULPIDEBUG_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [ulpidebug::W](W) writer structure
impl crate::Writable for ULPIDEBUG_SPEC {
    type Writer = W;
}
///`reset()` method sets ULPIDEBUG to value 0
