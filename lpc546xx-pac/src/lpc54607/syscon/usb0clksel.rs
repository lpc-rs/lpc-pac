...
///Field `SEL` reader - USB0 device clock source selection.
...
///Field `SEL` writer - USB0 device clock source selection.
pub struct SEL_W<'a> {
    w: &'a mut W,
}
impl<'a> SEL_W<'a> {
...
///`read()` method returns [usb0clksel::R](R) reader structure
impl crate::Readable for USB0CLKSEL_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [usb0clksel::W](W) writer structure
impl crate::Writable for USB0CLKSEL_SPEC {
    type Writer = W;
}
///`reset()` method sets USB0CLKSEL to value 0x07
