...
///Field `DIVSEL` reader - Divider select.
...
///Field `DIVSEL` writer - Divider select.
...
///Field `FREQSEL` reader - Frequency select.
...
///Field `FREQSEL` writer - Frequency select.
...
///`read()` method returns [wdtoscctrl::R](R) reader structure
impl crate::Readable for WDTOSCCTRL_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [wdtoscctrl::W](W) writer structure
impl crate::Writable for WDTOSCCTRL_SPEC {
    type Writer = W;
}
///`reset()` method sets WDTOSCCTRL to value 0xa0
