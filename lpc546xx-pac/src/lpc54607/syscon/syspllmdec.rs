...
///Field `MDEC` reader - Decoded M-divider coefficient value.
...
///Field `MDEC` writer - Decoded M-divider coefficient value.
pub struct MDEC_W<'a> {
    w: &'a mut W,
}
impl<'a> MDEC_W<'a> {
...
///Field `MREQ` reader - MDEC reload request.
...
///Field `MREQ` writer - MDEC reload request.
pub struct MREQ_W<'a> {
    w: &'a mut W,
}
impl<'a> MREQ_W<'a> {
...
///`read()` method returns [syspllmdec::R](R) reader structure
impl crate::Readable for SYSPLLMDEC_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [syspllmdec::W](W) writer structure
impl crate::Writable for SYSPLLMDEC_SPEC {
    type Writer = W;
}
///`reset()` method sets SYSPLLMDEC to value 0
