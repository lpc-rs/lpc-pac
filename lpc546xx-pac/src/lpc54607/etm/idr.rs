...
///Field `ImplementationRevision` reader - Implementation revision. The value of these bits is b0000, indicating implementation revision, 0.
...
///Field `MinorETMarchitectureVersion` reader - Minor ETM architecture version. The value of these bits is 0b0101, indicating minor architecture version number 5.
...
///Field `MajorETMarchitectureVersion` reader - Major ETM architecture version. The value of these bits is 0b0010, indicating major architecture version number 3, ETMv3.
...
///Field `ProcessorFamily` reader - Processor family. The value of these bits is 0b1111, indicating that the processor family is not identified in this register.
...
///Field `LoadPCfirst` reader - Load PC first. The value of this bit is 0, indicating that data tracing is not supported.
...
pub enum THUMBINSTRUCTIONTRACING_A {
    ...
    ///1: A 32-bit Thimb instruction is traced as a single instruction.
...
///Field `ThumbInstructionTracing` reader - 32-bit Thumb instruction tracing. The value of this bit is 1, indicating that a 32-bit Thumb instruction is traced as a single instruction.
...
///Field `SecurityExtensionSupport` reader - Security Extensions support. The value of this bit is 0, indicating that the ETM behaves as if the processor is in Secure state at all times.
...
pub enum BRANCHPACKETENCODING_A {
    ///0: The ETM implements the original branch packet encoding.
    BRANCHPACKETENCODING_0 = 0,
    ///1: The ETM implements the alternative branch packet encoding.
...
///Field `BranchPacketEncoding` reader - Branch packet encoding. The value of this bit is 1, indicating that alternative branch packet encoding is implemented.
...
///Field `ImplementorCode` reader - Implementor code. These bits identify ARM as the implementor of the processor. The value of these bits is 01000001.
