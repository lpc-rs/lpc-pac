...
///Field `SEL` reader - Clock source for main clock source selector A
...
///Field `SEL` writer - Clock source for main clock source selector A
pub struct SEL_W<'a> {
    w: &'a mut W,
}
impl<'a> SEL_W<'a> {
...
///`read()` method returns [mainclksela::R](R) reader structure
impl crate::Readable for MAINCLKSELA_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mainclksela::W](W) writer structure
impl crate::Writable for MAINCLKSELA_SPEC {
    type Writer = W;
}
///`reset()` method sets MAINCLKSELA to value 0
