...
///Field `FORCEWAKE` writer - Force peripheral clocking to stay on during Deep Sleep and Power-down modes. When 1, clocking to peripherals is prevented from being shut down when the CPU enters Deep Sleep and Power-down modes. This is intended to allow a coprocessor to continue operating while the main CPU(s) are shut down.
...
///Field `FCWAKE` reader - Wake for Flexcomms. When 1, any Flexcomm FIFO reaching the level specified by its own TXLVL will cause peripheral clocking to wake up temporarily while the related status is asserted.
...
///Field `FCWAKE` writer - Wake for Flexcomms. When 1, any Flexcomm FIFO reaching the level specified by its own TXLVL will cause peripheral clocking to wake up temporarily while the related status is asserted.
...
///Field `WAKEDMIC` reader - Wake for Digital Microphone. When 1, the digital microphone input FIFO reaching the level specified by TRIGLVL of either channel will cause peripheral clocking to wake up temporarily while the related status is asserted.
...
///Field `WAKEDMIC` writer - Wake for Digital Microphone. When 1, the digital microphone input FIFO reaching the level specified by TRIGLVL of either channel will cause peripheral clocking to wake up temporarily while the related status is asserted.
...
///`read()` method returns [hwwake::R](R) reader structure
impl crate::Readable for HWWAKE_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [hwwake::W](W) writer structure
impl crate::Writable for HWWAKE_SPEC {
    type Writer = W;
}
///`reset()` method sets HWWAKE to value 0
