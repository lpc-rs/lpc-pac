...
///Field `SEL` reader - DMIC (audio subsystem) clock source select.
...
///Field `SEL` writer - DMIC (audio subsystem) clock source select.
pub struct SEL_W<'a> {
    w: &'a mut W,
}
impl<'a> SEL_W<'a> {
...
///`read()` method returns [dmicclksel::R](R) reader structure
impl crate::Readable for DMICCLKSEL_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dmicclksel::W](W) writer structure
impl crate::Writable for DMICCLKSEL_SPEC {
    type Writer = W;
}
///`reset()` method sets DMICCLKSEL to value 0x07
