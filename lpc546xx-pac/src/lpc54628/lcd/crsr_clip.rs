...
///Field `CRSRCLIPX` reader - Cursor clip position for X direction.
...
///Field `CRSRCLIPX` writer - Cursor clip position for X direction.
...
///Field `CRSRCLIPY` reader - Cursor clip position for Y direction.
...
///Field `CRSRCLIPY` writer - Cursor clip position for Y direction.
...
impl R {
    ///Bits 0:5 - Cursor clip position for X direction.
    #[inline(always)]
    pub fn crsrclipx(&self) -> CRSRCLIPX_R {
        CRSRCLIPX_R::new((self.bits & 0x3f) as u8)
    }
    ///Bits 8:13 - Cursor clip position for Y direction.
    #[inline(always)]
...
///`read()` method returns [crsr_clip::R](R) reader structure
impl crate::Readable for CRSR_CLIP_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [crsr_clip::W](W) writer structure
impl crate::Writable for CRSR_CLIP_SPEC {
    type Writer = W;
}
///`reset()` method sets CRSR_CLIP to value 0
