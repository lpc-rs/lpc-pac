...
///Field `LPP` reader - Lines per panel.
...
///Field `LPP` writer - Lines per panel.
pub struct LPP_W<'a> {
    w: &'a mut W,
}
impl<'a> LPP_W<'a> {
...
///Field `VSW` reader - Vertical synchronization pulse width.
...
///Field `VSW` writer - Vertical synchronization pulse width.
pub struct VSW_W<'a> {
    w: &'a mut W,
}
impl<'a> VSW_W<'a> {
...
///`read()` method returns [timv::R](R) reader structure
impl crate::Readable for TIMV_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [timv::W](W) writer structure
impl crate::Writable for TIMV_SPEC {
    type Writer = W;
}
///`reset()` method sets TIMV to value 0
