...
///Field `TRRD` reader - Active bank A to active bank B latency 0x0 - 0xE = n + 1 clock cycles.
...
///Field `TRRD` writer - Active bank A to active bank B latency 0x0 - 0xE = n + 1 clock cycles.
pub struct TRRD_W<'a> {
    w: &'a mut W,
}
impl<'a> TRRD_W<'a> {
...
///`read()` method returns [dynamicrrd::R](R) reader structure
impl crate::Readable for DYNAMICRRD_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dynamicrrd::W](W) writer structure
impl crate::Writable for DYNAMICRRD_SPEC {
    type Writer = W;
}
///`reset()` method sets DYNAMICRRD to value 0x0f
