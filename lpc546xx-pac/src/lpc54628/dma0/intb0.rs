...
///Field `IB` reader - Interrupt B status for DMA channel n. Bit n corresponds to DMA channel n. The number of bits = number of DMA channels in this device. Other bits are reserved. 0 = the DMA channel interrupt B is not active. 1 = the DMA channel interrupt B is active.
...
///Field `IB` writer - Interrupt B status for DMA channel n. Bit n corresponds to DMA channel n. The number of bits = number of DMA channels in this device. Other bits are reserved. 0 = the DMA channel interrupt B is not active. 1 = the DMA channel interrupt B is active.
pub struct IB_W<'a> {
    w: &'a mut W,
}
impl<'a> IB_W<'a> {
...
///`read()` method returns [intb0::R](R) reader structure
impl crate::Readable for INTB0_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [intb0::W](W) writer structure
impl crate::Writable for INTB0_SPEC {
    type Writer = W;
}
///`reset()` method sets INTB0 to value 0
