///Register `DMA_CHx_TXDESC_LIST_ADDR` reader
...
///Register `DMA_CHx_TXDESC_LIST_ADDR` writer
...
///Field `STL` reader - Start of transmit list This field contains the base address of the first in the Transmit list.
...
///Field `STL` writer - Start of transmit list This field contains the base address of the first in the Transmit list.
pub struct STL_W<'a> {
    w: &'a mut W,
}
impl<'a> STL_W<'a> {
...
///`read()` method returns [dma_chx_txdesc_list_addr::R](R) reader structure
impl crate::Readable for DMA_CHX_TXDESC_LIST_ADDR_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dma_chx_txdesc_list_addr::W](W) writer structure
impl crate::Writable for DMA_CHX_TXDESC_LIST_ADDR_SPEC {
    type Writer = W;
}
///`reset()` method sets DMA_CHx_TXDESC_LIST_ADDR to value 0
