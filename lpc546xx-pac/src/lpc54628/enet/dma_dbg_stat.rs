...
///Field `AHSTS` reader - AHB Master Status When high, this bit indicates that the AHB master FSMs are in the non-idle state.
...
///Field `AHSTS` writer - AHB Master Status When high, this bit indicates that the AHB master FSMs are in the non-idle state.
...
///Field `RPS0` reader - DMA Channel 0 Receive Process State This field indicates the Rx DMA FSM state for Channel 0: 0x0: Stopped (Reset or Stop Receive Command issued) 0x1: Running (Fetching Rx Transfer ) 0x2: Reserved 0x3: Running (Waiting for Rx packet) 0x4: Suspended (Rx Unavailable) 0x5: Running (Closing the Rx) 0x6: Timestamp write state 0x7: Running (Transferring the received packet data from the Rx buffer to the system memory) This field does not generate an interrupt.
...
///Field `TPS0` reader - DMA Channel 0 Transmit Process State This field indicates the Tx DMA FSM state for Channel 0: 000: Stopped (Reset or Stop Transmit Command issued) 0x1: Running (Fetching Tx Transfer) 0x2: Running (Waiting for status) 0x3: Running (Reading Data from system memory buffer and queuing it to the Tx buffer (Tx FIFO)) 0x4: Timestamp write state 0x5: Reserved for future use 0x6: Suspended (Tx Unavailable or Tx Buffer Underflow) 0x7: Running (Closing Tx ) This field does not generate an interrupt.
...
///Field `RPS1` reader - DMA Channel 1 Receive Process State This field indicates the Rx DMA FSM state for Channel 1.
...
///Field `TPS1` reader - DMA Channel 1 Transmit Process State This field indicates the Tx DMA FSM state for Channel 1.
...
///`read()` method returns [dma_dbg_stat::R](R) reader structure
impl crate::Readable for DMA_DBG_STAT_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dma_dbg_stat::W](W) writer structure
impl crate::Writable for DMA_DBG_STAT_SPEC {
    type Writer = W;
}
///`reset()` method sets DMA_DBG_STAT to value 0
