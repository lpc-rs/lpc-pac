...
///Field `LPS` reader - (read) LocalPowerStatus The Root Hub does not support the local power status feature; thus, this bit is always read as 0.
...
///Field `LPS` writer - (read) LocalPowerStatus The Root Hub does not support the local power status feature; thus, this bit is always read as 0.
pub struct LPS_W<'a> {
    w: &'a mut W,
}
impl<'a> LPS_W<'a> {
...
///Field `OCI` reader - OverCurrentIndicator This bit reports overcurrent conditions when the global reporting is implemented.
...
///Field `OCI` writer - OverCurrentIndicator This bit reports overcurrent conditions when the global reporting is implemented.
pub struct OCI_W<'a> {
    w: &'a mut W,
}
impl<'a> OCI_W<'a> {
...
///Field `DRWE` reader - (read) DeviceRemoteWakeupEnable This bit enables a ConnectStatusChange bit as a resume event, causing a USBSUSPEND to USBRESUME state transition and setting the ResumeDetected interrupt.
...
///Field `DRWE` writer - (read) DeviceRemoteWakeupEnable This bit enables a ConnectStatusChange bit as a resume event, causing a USBSUSPEND to USBRESUME state transition and setting the ResumeDetected interrupt.
pub struct DRWE_W<'a> {
    w: &'a mut W,
}
impl<'a> DRWE_W<'a> {
...
///Field `LPSC` reader - (read) LocalPowerStatusChange The root hub does not support the local power status feature.
...
///Field `LPSC` writer - (read) LocalPowerStatusChange The root hub does not support the local power status feature.
pub struct LPSC_W<'a> {
    w: &'a mut W,
}
impl<'a> LPSC_W<'a> {
...
///Field `OCIC` reader - OverCurrentIndicatorChange This bit is set by hardware when a change has occurred to the OCI field of this register.
...
///Field `OCIC` writer - OverCurrentIndicatorChange This bit is set by hardware when a change has occurred to the OCI field of this register.
pub struct OCIC_W<'a> {
    w: &'a mut W,
}
impl<'a> OCIC_W<'a> {
...
///Field `CRWE` reader - (write) ClearRemoteWakeupEnable Writing a 1 clears DeviceRemoveWakeupEnable.
...
///Field `CRWE` writer - (write) ClearRemoteWakeupEnable Writing a 1 clears DeviceRemoveWakeupEnable.
pub struct CRWE_W<'a> {
    w: &'a mut W,
}
impl<'a> CRWE_W<'a> {
...
///`read()` method returns [hcrhstatus::R](R) reader structure
impl crate::Readable for HCRHSTATUS_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [hcrhstatus::W](W) writer structure
impl crate::Writable for HCRHSTATUS_SPEC {
    type Writer = W;
}
///`reset()` method sets HCRHSTATUS to value 0
