///Register `HCRHDESCRIPTORA` reader
...
///Field `NDP` reader - NumberDownstreamPorts These bits specify the number of downstream ports supported by the root hub.
...
///Field `NDP` writer - NumberDownstreamPorts These bits specify the number of downstream ports supported by the root hub.
pub struct NDP_W<'a> {
    w: &'a mut W,
}
impl<'a> NDP_W<'a> {
...
///Field `PSM` reader - PowerSwitchingMode This bit is used to specify how the power switching of the root hub ports is controlled.
...
///Field `PSM` writer - PowerSwitchingMode This bit is used to specify how the power switching of the root hub ports is controlled.
pub struct PSM_W<'a> {
    w: &'a mut W,
}
impl<'a> PSM_W<'a> {
...
///Field `NPS` reader - NoPowerSwitching These bits are used to specify whether power switching is supported or port are always powered.
...
///Field `NPS` writer - NoPowerSwitching These bits are used to specify whether power switching is supported or port are always powered.
pub struct NPS_W<'a> {
    w: &'a mut W,
}
impl<'a> NPS_W<'a> {
...
///Field `DT` reader - DeviceType This bit specifies that the root hub is not a compound device.
...
///Field `DT` writer - DeviceType This bit specifies that the root hub is not a compound device.
pub struct DT_W<'a> {
    w: &'a mut W,
}
impl<'a> DT_W<'a> {
...
///Field `OCPM` reader - OverCurrentProtectionMode This bit describes how the overcurrent status for the root hub ports are reported.
...
///Field `OCPM` writer - OverCurrentProtectionMode This bit describes how the overcurrent status for the root hub ports are reported.
pub struct OCPM_W<'a> {
    w: &'a mut W,
}
impl<'a> OCPM_W<'a> {
...
///Field `NOCP` reader - NoOverCurrentProtection This bit describes how the overcurrent status for the root hub ports are reported.
...
///Field `NOCP` writer - NoOverCurrentProtection This bit describes how the overcurrent status for the root hub ports are reported.
pub struct NOCP_W<'a> {
    w: &'a mut W,
}
impl<'a> NOCP_W<'a> {
...
///Field `POTPGT` reader - PowerOnToPowerGoodTime This byte specifies the duration the HCD has to wait before accessing a powered-on port of the root hub.
...
///Field `POTPGT` writer - PowerOnToPowerGoodTime This byte specifies the duration the HCD has to wait before accessing a powered-on port of the root hub.
...
///`read()` method returns [hcrhdescriptora::R](R) reader structure
impl crate::Readable for HCRHDESCRIPTORA_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [hcrhdescriptora::W](W) writer structure
impl crate::Writable for HCRHDESCRIPTORA_SPEC {
    type Writer = W;
}
///`reset()` method sets HCRHDESCRIPTORA to value 0xff00_0902
