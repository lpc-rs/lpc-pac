...
///Field `ALARM1HZ` reader - RTC 1 Hz timer alarm flag status.
...
///Field `ALARM1HZ` writer - RTC 1 Hz timer alarm flag status.
...
///Field `WAKE1KHZ` reader - RTC 1 kHz timer wake-up flag status.
...
///Field `WAKE1KHZ` writer - RTC 1 kHz timer wake-up flag status.
...
///Field `ALARMDPD_EN` reader - RTC 1 Hz timer alarm enable for Deep power-down.
...
///Field `ALARMDPD_EN` writer - RTC 1 Hz timer alarm enable for Deep power-down.
...
///Field `WAKEDPD_EN` reader - RTC 1 kHz timer wake-up enable for Deep power-down.
...
///Field `WAKEDPD_EN` writer - RTC 1 kHz timer wake-up enable for Deep power-down.
...
///Field `RTC1KHZ_EN` reader - RTC 1 kHz clock enable. This bit can be set to 0 to conserve power if the 1 kHz timer is not used. This bit has no effect when the RTC is disabled (bit 7 of this register is 0).
...
///Field `RTC_EN` reader - RTC enable.
...
///Field `RTC_EN` writer - RTC enable.
