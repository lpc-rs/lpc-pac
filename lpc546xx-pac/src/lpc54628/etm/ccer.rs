...
///Field `ExtendedExternalInputSelectors` reader - Extended external input selectors. The value of these bits is 0, indicating that extended external input selectors are not implemented.
...
///Field `ExtendedExternalInputBus` reader - Extended external input bus. The value of these bits is 0, indicating that the extended external input bus is not implemented.
...
///Field `ReadableRegisters` reader - Readable registers. The value of this bit is 1, indicating that all registers are readable.
...
///Field `DataAddressComparisons` reader - Data address comparisons. The value of this bit is 1, indicating that data address comparisons are not supported.
...
///Field `InstrumentationResources` reader - Instrumentation resources. The value of these bits is 0b000, indicating that no Instrumentation resources are supported.
...
///Field `EmbeddedICEwatchpointInputs` reader - EmbeddedICE watchpoint inputs. The value of these bits is 0b0100, indicating that the number of EmbeddedICE watchpoint inputs implemented is four. These inputs come from the DWT.
...
///Field `TraceStartStopBlockUsesEmbeddedICEwatchpointInputs` reader - Trace Start/Stop block uses EmbeddedICE watchpoint inputs. The value of this bit is 1, indicating that the Trace Start/Stop block uses the EmbeddedICE watchpoint inputs.
...
///Field `EmbeddedICEbehaviorControlImplemented` reader - EmbeddedICE behavior control implemented. The value of this bit is 0, indicating that the ETMEIBCR is not implemented.
...
///Field `TimestampingImplemented` reader - Timestamping implemented. This bit is set to 1, indicating that timestamping is implemented.
...
///Field `ReducedFunctionCounter` reader - Reduced function counter. Set to 1 to indicate that Counter 1 is a reduced function counter.
...
///Field `TimestampEncoding` reader - Timestamp encoding. Set to 1 to indicate that the timestamp is encoded as a natural binary number.
...
///Field `TimestampSize` reader - Timestamp size. Set to 0 to indicate a size of 48 bits.
