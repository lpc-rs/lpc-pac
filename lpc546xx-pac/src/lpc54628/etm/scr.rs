...
///Field `MaximumPortSize` reader - Maximum ETM port size bits \[2:0\]. These bits are used in conjunction with bit \[9\]. The value of these bits is b001.
...
///Field `FIFOFULLsupported` reader - FIFOFULL supported. The value of this bit is 1, indicating that FIFOFULL is supported. This bit is used in conjunction with bit \[23\]
///of the ETMCCR.
...
///Field `MaximumPortSize3` reader - Maximum ETM port size bit \[3\]. This bit is used in conjunction with bits \[2:0\]. Its value is 0. This has no effect on the TPIU trace port.
...
///Field `PortSizeSupported` reader - Port size supported. This bit reads as 1 if the currently selected port size is supported. This has no effect on the TPIU trace port.
...
///Field `PortModeSupported` reader - Port mode supported. This bit reads as 1 if the currently selected port mode is supported. This has no effect on the TPIU trace port.
...
///Field `N` reader - These bits give the number of supported processors minus 1. The value of these bits is b000, indicating that there is only one processor connected.
...
///Field `NoFetchComparisons` reader - No Fetch comparisons. The value of this bit is 1, indicating that fetch comparisons are not implemented.
...
impl R {
    ...
    ///Bit 8 - FIFOFULL supported. The value of this bit is 1, indicating that FIFOFULL is supported. This bit is used in conjunction with bit \[23\]
    ///of the ETMCCR.
    #[inline(always)]
