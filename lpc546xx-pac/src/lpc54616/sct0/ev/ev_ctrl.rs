...
pub enum DIRECTION_A {
    ///0: Direction independent. This event is triggered regardless of the count direction.
    DIRECTION_INDEPENDENT = 0,
    ///1: Counting up. This event is triggered only during up-counting when BIDIR = 1.
    COUNTING_UP = 1,
    ///2: Counting down. This event is triggered only during down-counting when BIDIR = 1.
    COUNTING_DOWN = 2,
}
...
///`read()` method returns [ev_ctrl::R](R) reader structure
impl crate::Readable for EV_CTRL_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [ev_ctrl::W](W) writer structure
impl crate::Writable for EV_CTRL_SPEC {
    type Writer = W;
}
///`reset()` method sets EV_CTRL to value 0
