...
///Field `STATEMSKn` reader - If bit m is one, event n happens in state m of the counter selected by the HEVENT bit (n = event number, m = state number; state 0 = bit 0, state 1= bit 1, etc.). The number of bits = number of states in this SCT.
...
///Field `STATEMSKn` writer - If bit m is one, event n happens in state m of the counter selected by the HEVENT bit (n = event number, m = state number; state 0 = bit 0, state 1= bit 1, etc.). The number of bits = number of states in this SCT.
...
///`read()` method returns [ev_state::R](R) reader structure
impl crate::Readable for EV_STATE_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [ev_state::W](W) writer structure
impl crate::Writable for EV_STATE_SPEC {
    type Writer = W;
}
///`reset()` method sets EV_STATE to value 0
