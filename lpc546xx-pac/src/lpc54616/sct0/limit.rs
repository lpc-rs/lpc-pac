...
///Field `LIMMSK_L` reader - If bit n is one, event n is used as a counter limit for the L or unified counter (event 0 = bit 0, event 1 = bit 1, etc.). The number of bits = number of events in this SCT.
...
///Field `LIMMSK_L` writer - If bit n is one, event n is used as a counter limit for the L or unified counter (event 0 = bit 0, event 1 = bit 1, etc.). The number of bits = number of events in this SCT.
...
///Field `LIMMSK_H` reader - If bit n is one, event n is used as a counter limit for the H counter (event 0 = bit 16, event 1 = bit 17, etc.). The number of bits = number of events in this SCT.
...
///Field `LIMMSK_H` writer - If bit n is one, event n is used as a counter limit for the H counter (event 0 = bit 16, event 1 = bit 17, etc.). The number of bits = number of events in this SCT.
...
///`read()` method returns [limit::R](R) reader structure
impl crate::Readable for LIMIT_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [limit::W](W) writer structure
impl crate::Writable for LIMIT_SPEC {
    type Writer = W;
}
///`reset()` method sets LIMIT to value 0
