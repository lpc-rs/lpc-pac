...
///Field `RMode` reader - Default value for FPSCR.RMode (Rounding Mode control field).
...
///`read()` method returns [fpdscr::R](R) reader structure
impl crate::Readable for FPDSCR_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [fpdscr::W](W) writer structure
