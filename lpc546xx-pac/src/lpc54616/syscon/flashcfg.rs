...
#[repr(u8)]
pub enum FETCHCFG_A {
    ///0: Instruction fetches from flash are not buffered. Every fetch request from the CPU results in a read of the flash memory. This setting may use significantly more power than when buffering is enabled.
    NO_BUFFER = 0,
    ///1: One buffer is used for all instruction fetches.
    ONE_BUFFER = 1,
    ///2: All buffers may be used for instruction fetches.
    ALL_BUFFERS = 2,
}
...
///Field `FETCHCFG` reader - Instruction fetch configuration. This field determines how flash accelerator buffers are used for instruction fetches.
...
///Field `FETCHCFG` writer - Instruction fetch configuration. This field determines how flash accelerator buffers are used for instruction fetches.
...
#[repr(u8)]
pub enum DATACFG_A {
    ///0: Data accesses from flash are not buffered. Every data access from the CPU results in a read of the flash memory.
    NOT_BUFFERED = 0,
    ///1: One buffer is used for all data accesses.
    ONE_BUFFER = 1,
    ///2: All buffers may be used for data accesses.
    ALL_BUFFERS = 2,
}
...
///Field `DATACFG` reader - Data read configuration. This field determines how flash accelerator buffers are used for data accesses.
...
///Field `DATACFG` writer - Data read configuration. This field determines how flash accelerator buffers are used for data accesses.
...
///Field `ACCEL` reader - Acceleration enable.
...
///Field `ACCEL` writer - Acceleration enable.
...
///Field `PREFEN` reader - Prefetch enable.
...
///Field `PREFEN` writer - Prefetch enable.
...
///Field `PREFOVR` reader - Prefetch override. This bit only applies when PREFEN = 1 and a buffered instruction is completing for which the next flash line is not already buffered or being prefetched.
...
///Field `PREFOVR` writer - Prefetch override. This bit only applies when PREFEN = 1 and a buffered instruction is completing for which the next flash line is not already buffered or being prefetched.
...
#[repr(u8)]
pub enum FLASHTIM_A {
    ///0: 1 system clock flash access time (for system clock rates up to 12 MHz).
    N_1_CLOCK_CYCLE = 0,
    ///1: 2 system clocks flash access time (for system clock rates up to 30 MHz).
    N_2_CLOCK_CYCLES = 1,
    ///2: 3 system clocks flash access time (for system clock rates up to 60 MHz).
    N_3_CLOCK_CYCLES = 2,
    ///3: 4 system clocks flash access time (for system clock rates up to 85 MHz).
    N_4_CLOCK_CYCLES = 3,
    ///4: 5 system clocks flash access time (for system clock rates up to 100 MHz).
    N_5_CLOCK_CYCLES = 4,
}
...
///Field `FLASHTIM` reader - Flash memory access time. The number of system clocks used for flash accesses is equal to FLASHTIM +1.
...
///Field `FLASHTIM` writer - Flash memory access time. The number of system clocks used for flash accesses is equal to FLASHTIM +1.
...
///`read()` method returns [flashcfg::R](R) reader structure
impl crate::Readable for FLASHCFG_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [flashcfg::W](W) writer structure
impl crate::Writable for FLASHCFG_SPEC {
    type Writer = W;
}
///`reset()` method sets FLASHCFG to value 0x0d1a
