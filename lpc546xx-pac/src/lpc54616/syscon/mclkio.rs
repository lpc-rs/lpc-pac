...
///Field `DIR` reader - MCLK direction control.
...
///Field `DIR` writer - MCLK direction control.
pub struct DIR_W<'a> {
    w: &'a mut W,
}
impl<'a> DIR_W<'a> {
...
///`read()` method returns [mclkio::R](R) reader structure
impl crate::Readable for MCLKIO_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mclkio::W](W) writer structure
impl crate::Writable for MCLKIO_SPEC {
    type Writer = W;
}
///`reset()` method sets MCLKIO to value 0
