///Register `PRESETCTRLCLR[%s]` writer
...
///Field `RST_CLR` writer - Writing ones to this register clears the corresponding bit or bits in the PRESETCTRLn register, if they are implemented. Bits that do not correspond to defined bits in PRESETCTRLn are reserved and only zeroes should be written to them.
...
///`write(|w| ..)` method takes [presetctrlclr::W](W) writer structure
impl crate::Writable for PRESETCTRLCLR_SPEC {
    type Writer = W;
}
///`reset()` method sets PRESETCTRLCLR[%s]
///to value 0
