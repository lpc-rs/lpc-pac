...
///Field `START_SET` writer - Writing ones to this register sets the corresponding bit or bits in the STARTER0 register, if they are implemented.
...
///`write(|w| ..)` method takes [starterset::W](W) writer structure
impl crate::Writable for STARTERSET_SPEC {
    type Writer = W;
}
///`reset()` method sets STARTERSET[%s]
///to value 0
