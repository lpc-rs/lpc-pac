///Register `MAC_EXT_CONFIG` reader
...
///Register `MAC_EXT_CONFIG` writer
...
///Field `GPSL` reader - Giant Packet Size Limit If the received packet size is greater than the value programmed in this field in units of bytes, the MAC declares the received packet as Giant packet.
...
///Field `GPSL` writer - Giant Packet Size Limit If the received packet size is greater than the value programmed in this field in units of bytes, the MAC declares the received packet as Giant packet.
pub struct GPSL_W<'a> {
    w: &'a mut W,
}
impl<'a> GPSL_W<'a> {
...
///Field `DCRCC` reader - Disable CRC Checking for Received Packets When this bit is set, the MAC receiver does not check the CRC field in the received packets.
...
///Field `DCRCC` writer - Disable CRC Checking for Received Packets When this bit is set, the MAC receiver does not check the CRC field in the received packets.
...
///Field `SPEN` reader - Slow Protocol Detection Enable When this bit is set, MAC processes the Slow Protocol packets (Ether Type 0x8809) and provides the Rx status.
...
///Field `SPEN` writer - Slow Protocol Detection Enable When this bit is set, MAC processes the Slow Protocol packets (Ether Type 0x8809) and provides the Rx status.
pub struct SPEN_W<'a> {
    w: &'a mut W,
}
impl<'a> SPEN_W<'a> {
...
///Field `USP` reader - Unicast Slow Protocol Packet Detect When this bit is set, the MAC detects the Slow Protocol packets with unicast address of the station specified in the MAC Address High Table 747 and MAC Address Low Table 748 registers.
...
///Field `USP` writer - Unicast Slow Protocol Packet Detect When this bit is set, the MAC detects the Slow Protocol packets with unicast address of the station specified in the MAC Address High Table 747 and MAC Address Low Table 748 registers.
pub struct USP_W<'a> {
    w: &'a mut W,
}
impl<'a> USP_W<'a> {
...
///`read()` method returns [mac_ext_config::R](R) reader structure
impl crate::Readable for MAC_EXT_CONFIG_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mac_ext_config::W](W) writer structure
impl crate::Writable for MAC_EXT_CONFIG_SPEC {
    type Writer = W;
}
///`reset()` method sets MAC_EXT_CONFIG to value 0
