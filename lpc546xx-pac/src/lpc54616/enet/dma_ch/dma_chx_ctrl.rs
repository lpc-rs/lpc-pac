...
///Field `DSL` reader - Skip Length This bit specifies the Word, Dword, or Lword number (depending on the 32- bit, 64-bit, or 128-bit bus) to skip between two unchained s.
...
///Field `DSL` writer - Skip Length This bit specifies the Word, Dword, or Lword number (depending on the 32- bit, 64-bit, or 128-bit bus) to skip between two unchained s.
pub struct DSL_W<'a> {
    w: &'a mut W,
}
impl<'a> DSL_W<'a> {
...
///`read()` method returns [dma_chx_ctrl::R](R) reader structure
impl crate::Readable for DMA_CHX_CTRL_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dma_chx_ctrl::W](W) writer structure
impl crate::Writable for DMA_CHX_CTRL_SPEC {
    type Writer = W;
}
///`reset()` method sets DMA_CHx_CTRL to value 0
