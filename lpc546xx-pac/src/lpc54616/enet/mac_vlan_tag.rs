...
///Field `VTHM` reader - Disable VLAN Type Check.
...
///Field `VTHM` writer - Disable VLAN Type Check.
pub struct VTHM_W<'a> {
    w: &'a mut W,
}
impl<'a> VTHM_W<'a> {
...
///`read()` method returns [mac_vlan_tag::R](R) reader structure
impl crate::Readable for MAC_VLAN_TAG_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mac_vlan_tag::W](W) writer structure
impl crate::Writable for MAC_VLAN_TAG_SPEC {
    type Writer = W;
}
///`reset()` method sets MAC_VLAN_TAG to value 0
