...
///Field `IA` reader - Interrupt A status for DMA channel n. Bit n corresponds to DMA channel n. The number of bits = number of DMA channels in this device. Other bits are reserved. 0 = the DMA channel interrupt A is not active. 1 = the DMA channel interrupt A is active.
...
///Field `IA` writer - Interrupt A status for DMA channel n. Bit n corresponds to DMA channel n. The number of bits = number of DMA channels in this device. Other bits are reserved. 0 = the DMA channel interrupt A is not active. 1 = the DMA channel interrupt A is active.
pub struct IA_W<'a> {
    w: &'a mut W,
}
impl<'a> IA_W<'a> {
...
///`read()` method returns [inta0::R](R) reader structure
impl crate::Readable for INTA0_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [inta0::W](W) writer structure
impl crate::Writable for INTA0_SPEC {
    type Writer = W;
}
///`reset()` method sets INTA0 to value 0
