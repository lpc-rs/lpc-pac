...
///Field `FIFO_RX_WATERMARK` writer - FIFO reached Receive watermark level; not qualified with data transfer.
...
///Field `FIFO_TX_WATERMARK` writer - FIFO reached Transmit watermark level; not qualified with data transfer.
...
///Field `FIFO_EMPTY` writer - FIFO is empty status.
...
///Field `FIFO_FULL` writer - FIFO is full status.
...
///Field `CMDFSMSTATES` writer - Command FSM states: 0 - Idle 1 - Send init sequence 2 - Tx cmd start bit 3 - Tx cmd tx bit 4 - Tx cmd index + arg 5 - Tx cmd crc7 6 - Tx cmd end bit 7 - Rx resp start bit 8 - Rx resp IRQ response 9 - Rx resp tx bit 10 - Rx resp cmd idx 11 - Rx resp data 12 - Rx resp crc7 13 - Rx resp end bit 14 - Cmd path wait NCC 15 - Wait; CMD-to-response turnaround NOTE: The command FSM state is represented using 19 bits.
...
///Field `DATA_3_STATUS` reader - Raw selected card_data\[3\]; checks whether card is present 0 - card not present 1 - card present.
...
///Field `DATA_3_STATUS` writer - Raw selected card_data\[3\]; checks whether card is present 0 - card not present 1 - card present.
...
///Field `DATA_BUSY` reader - Inverted version of raw selected card_data\[0\]
...
///Field `DATA_BUSY` writer - Inverted version of raw selected card_data\[0\]
///0 - card data not busy 1 - card data busy.
...
///Field `DATA_STATE_MC_BUSY` writer - Data transmit or receive state-machine is busy.
...
///Field `RESPONSE_INDEX` writer - Index of previous response, including any auto-stop sent by core.
...
///Field `FIFO_COUNT` writer - FIFO count - Number of filled locations in FIFO.
...
///Field `DMA_ACK` reader - DMA acknowledge signal state.
...
///Field `DMA_ACK` writer - DMA acknowledge signal state.
...
///Field `DMA_REQ` reader - DMA request signal state.
...
///Field `DMA_REQ` writer - DMA request signal state.
...
impl R {
    ...
    ///Bit 9 - Inverted version of raw selected card_data\[0\]
    ///0 - card data not busy 1 - card data busy.
    #[inline(always)]
...
///`reset()` method sets STATUS to value 0x0406
