...
///Field `NOTP` writer - Toggle output bits (bit 0 = PIOn_0, bit 1 = PIOn_1, etc.). Supported pins depends on the specific device and package. 0 = no operation. 1 = Toggle output bit.
pub struct NOTP_W<'a> {
    w: &'a mut W,
}
impl<'a> NOTP_W<'a> {
...
///`write(|w| ..)` method takes [not::W](W) writer structure
impl crate::Writable for NOT_SPEC {
    type Writer = W;
}
///`reset()` method sets NOT[%s]
///to value 0
