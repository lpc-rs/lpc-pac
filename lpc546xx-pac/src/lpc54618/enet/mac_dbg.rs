...
///Field `REPESTS` reader - MAC MII Receive Protocol Engine Status When this bit is set, it indicates that the MAC MII receive protocol engine is actively receiving data, and it is not in the Idle state.
...
///Field `TPESTS` reader - MAC MII Transmit Protocol Engine Status When this bit is set, it indicates that the MAC or MII transmit protocol engine is actively transmitting data, and it is not in the Idle state.
...
///Field `TFCSTS` reader - MAC Transmit Packet Controller Status This field indicates the state of the MAC Transmit Packet Controller module.
...
///`read()` method returns [mac_dbg::R](R) reader structure
impl crate::Readable for MAC_DBG_SPEC {
    type Reader = R;
}
///`reset()` method sets MAC_DBG to value 0
