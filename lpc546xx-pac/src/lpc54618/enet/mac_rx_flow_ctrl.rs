///Register `MAC_RX_FLOW_CTRL` reader
...
///Register `MAC_RX_FLOW_CTRL` writer
...
///Field `RFE` reader - Receive Flow Control Enable When this bit is set and the MAC is operating in full-duplex mode, the MAC decodes the received Pause packet and disables its transmitter for a specified (Pause) time.
...
///Field `RFE` writer - Receive Flow Control Enable When this bit is set and the MAC is operating in full-duplex mode, the MAC decodes the received Pause packet and disables its transmitter for a specified (Pause) time.
pub struct RFE_W<'a> {
    w: &'a mut W,
}
impl<'a> RFE_W<'a> {
...
///Field `UP` reader - Unicast Pause Packet Detect A pause packet is processed when it has the unique multicast address specified in the IEEE 802.
...
///Field `UP` writer - Unicast Pause Packet Detect A pause packet is processed when it has the unique multicast address specified in the IEEE 802.
pub struct UP_W<'a> {
    w: &'a mut W,
}
impl<'a> UP_W<'a> {
...
///`read()` method returns [mac_rx_flow_ctrl::R](R) reader structure
impl crate::Readable for MAC_RX_FLOW_CTRL_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mac_rx_flow_ctrl::W](W) writer structure
impl crate::Writable for MAC_RX_FLOW_CTRL_SPEC {
    type Writer = W;
}
///`reset()` method sets MAC_RX_FLOW_CTRL to value 0
