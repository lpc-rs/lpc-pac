///Register `DMA_CHx_RX_CTRL` reader
...
///Register `DMA_CHx_RX_CTRL` writer
...
///Field `SR` reader - Start or Stop Receive When this bit is set, the DMA tries to acquire the from the receive list and processes the incoming packets.
...
///Field `SR` writer - Start or Stop Receive When this bit is set, the DMA tries to acquire the from the receive list and processes the incoming packets.
pub struct SR_W<'a> {
    w: &'a mut W,
}
impl<'a> SR_W<'a> {
...
///Field `RBSZ` reader - Receive Buffer size This field indicates the size of the Rx buffers specified in bytes.
...
///Field `RBSZ` writer - Receive Buffer size This field indicates the size of the Rx buffers specified in bytes.
pub struct RBSZ_W<'a> {
    w: &'a mut W,
}
impl<'a> RBSZ_W<'a> {
...
///Field `RxPBL` reader - Receive Programmable Burst Length These bits indicate the maximum number of beats to be transferred in one DMA data transfer.
...
///Field `RxPBL` writer - Receive Programmable Burst Length These bits indicate the maximum number of beats to be transferred in one DMA data transfer.
...
///Field `RPF` reader - DMA Rx Channel 0 Packet Flush When this bit is set to 1, the DMA will automatically flush the packet from the Rx Queues destined to DMA Rx Channel 0 when the DMA Rx Channel 0 is stopped after a system bus error has occurred.
...
///Field `RPF` writer - DMA Rx Channel 0 Packet Flush When this bit is set to 1, the DMA will automatically flush the packet from the Rx Queues destined to DMA Rx Channel 0 when the DMA Rx Channel 0 is stopped after a system bus error has occurred.
pub struct RPF_W<'a> {
    w: &'a mut W,
}
impl<'a> RPF_W<'a> {
...
///`read()` method returns [dma_chx_rx_ctrl::R](R) reader structure
impl crate::Readable for DMA_CHX_RX_CTRL_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dma_chx_rx_ctrl::W](W) writer structure
impl crate::Writable for DMA_CHX_RX_CTRL_SPEC {
    type Writer = W;
}
///`reset()` method sets DMA_CHx_RX_CTRL to value 0
