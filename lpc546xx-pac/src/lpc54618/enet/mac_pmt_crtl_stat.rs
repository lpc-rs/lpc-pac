///Register `MAC_PMT_CRTL_STAT` reader
...
///Register `MAC_PMT_CRTL_STAT` writer
...
///Field `PWRDWN` reader - Transmit LPI Entry When this bit is set, it indicates that the MAC Transmitter has entered the LPI state because of the setting of the LPIEN bit.
...
///Field `RWKPKTEN` reader - Remote Wake-Up Packet Enable When this bit is set, a power management event is generated when the MAC receives a remote wake-up packet.
...
///Field `GLBLUCAST` reader - Global Unicast When this bit set, any unicast packet filtered by the MAC (DAF) address recognition is detected as a remote wake-up packet.
...
///Field `GLBLUCAST` writer - Global Unicast When this bit set, any unicast packet filtered by the MAC (DAF) address recognition is detected as a remote wake-up packet.
...
///Field `RWKPFE` reader - Remote Wake-up Packet Forwarding Enable When this bit is set along with RWKPKTEN, the MAC receiver drops all received frames until it receives the expected wake-up frame.
...
///Field `RWKPFE` writer - Remote Wake-up Packet Forwarding Enable When this bit is set along with RWKPKTEN, the MAC receiver drops all received frames until it receives the expected wake-up frame.
...
///Field `RWKPTR` reader - Remote Wake-up FIFO Pointer This field gives the current value (0 to 7) of the Remote Wake-up Packet Filter register pointer.
...
///Field `RWKPTR` writer - Remote Wake-up FIFO Pointer This field gives the current value (0 to 7) of the Remote Wake-up Packet Filter register pointer.
...
///Field `RWKFILTRST` reader - Remote Wake-Up Packet Filter Register Pointer Reset When this bit is set, the remote wake-up packet filter register pointer is reset to 3'b000.
...
///Field `RWKFILTRST` writer - Remote Wake-Up Packet Filter Register Pointer Reset When this bit is set, the remote wake-up packet filter register pointer is reset to 3'b000.
...
///`read()` method returns [mac_pmt_crtl_stat::R](R) reader structure
impl crate::Readable for MAC_PMT_CRTL_STAT_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mac_pmt_crtl_stat::W](W) writer structure
impl crate::Writable for MAC_PMT_CRTL_STAT_SPEC {
    type Writer = W;
}
///`reset()` method sets MAC_PMT_CRTL_STAT to value 0
