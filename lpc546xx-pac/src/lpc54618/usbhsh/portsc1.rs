...
///Field `CCS` reader - Current Connect Status: Logic 1 indicates a device is present on the port.
...
///Field `CCS` writer - Current Connect Status: Logic 1 indicates a device is present on the port.
pub struct CCS_W<'a> {
    w: &'a mut W,
}
impl<'a> CCS_W<'a> {
...
///Field `CSC` reader - Connect Status Change: Logic 1 means that the value of CCS has changed.
...
///Field `CSC` writer - Connect Status Change: Logic 1 means that the value of CCS has changed.
pub struct CSC_W<'a> {
    w: &'a mut W,
}
impl<'a> CSC_W<'a> {
...
///Field `PED` reader - Port Enabled/Disabled.
...
///Field `PED` writer - Port Enabled/Disabled.
pub struct PED_W<'a> {
    w: &'a mut W,
}
impl<'a> PED_W<'a> {
...
///Field `PEDC` reader - Port Enabled/Disabled Change: Logic 1 means that the value of PED has changed.
...
///Field `PEDC` writer - Port Enabled/Disabled Change: Logic 1 means that the value of PED has changed.
pub struct PEDC_W<'a> {
    w: &'a mut W,
}
impl<'a> PEDC_W<'a> {
...
///Field `OCA` reader - Over-current active: Logic 1 means that this port has an over-current condition.
...
///Field `OCA` writer - Over-current active: Logic 1 means that this port has an over-current condition.
pub struct OCA_W<'a> {
    w: &'a mut W,
}
impl<'a> OCA_W<'a> {
...
///Field `OCC` reader - Over-current change: Logic 1 means that the value of OCA has changed.
...
///Field `OCC` writer - Over-current change: Logic 1 means that the value of OCA has changed.
pub struct OCC_W<'a> {
    w: &'a mut W,
}
impl<'a> OCC_W<'a> {
...
///Field `FPR` reader - Force Port Resume: Logic 1 means resume (K-state) detected or driven on the port.
...
///Field `FPR` writer - Force Port Resume: Logic 1 means resume (K-state) detected or driven on the port.
pub struct FPR_W<'a> {
    w: &'a mut W,
}
impl<'a> FPR_W<'a> {
...
///Field `SUSP` reader - Suspend: Logic 1 means port is in the suspend state.
...
///Field `SUSP` writer - Suspend: Logic 1 means port is in the suspend state.
pub struct SUSP_W<'a> {
    w: &'a mut W,
}
impl<'a> SUSP_W<'a> {
...
///Field `PR` reader - Port Reset: Logic 1 means the port is in the reset state.
...
///Field `PR` writer - Port Reset: Logic 1 means the port is in the reset state.
pub struct PR_W<'a> {
    w: &'a mut W,
}
impl<'a> PR_W<'a> {
...
///Field `SUS_L1` reader - Suspend using L1 0b = Suspend using L2 1b = Suspend using L1 When this bit is set to a 1 and a non-zero value is specified in the Device Address field, the host controller will generate an LPM Token to enter the L1 state whenever software writes a one to the Suspend bit, as well as L1 exit timing during any device or host-initiated resume.
...
///Field `SUS_L1` writer - Suspend using L1 0b = Suspend using L2 1b = Suspend using L1 When this bit is set to a 1 and a non-zero value is specified in the Device Address field, the host controller will generate an LPM Token to enter the L1 state whenever software writes a one to the Suspend bit, as well as L1 exit timing during any device or host-initiated resume.
...
///Field `LS` reader - Line Status: This field reflects the current logical levels of the DP (bit 11) and DM (bit 10) signal lines.
...
///Field `PP` reader - Port Power: The function of this bit depends on the value of the Port Power Control (PPC) bit in the HCSPARAMS register.
...
///Field `PP` writer - Port Power: The function of this bit depends on the value of the Port Power Control (PPC) bit in the HCSPARAMS register.
pub struct PP_W<'a> {
    w: &'a mut W,
}
impl<'a> PP_W<'a> {
...
///Field `PIC` reader - Port Indicator Control : Writing to this field has no effect if the P_INDICATOR bit in the HCSPARAMS register is logic 0.
...
///Field `PIC` writer - Port Indicator Control : Writing to this field has no effect if the P_INDICATOR bit in the HCSPARAMS register is logic 0.
pub struct PIC_W<'a> {
    w: &'a mut W,
}
impl<'a> PIC_W<'a> {
...
///Field `PTC` reader - Port Test Control: A non-zero value indicates that the port is operating in the test mode as indicated by the value.
...
///Field `PTC` writer - Port Test Control: A non-zero value indicates that the port is operating in the test mode as indicated by the value.
pub struct PTC_W<'a> {
    w: &'a mut W,
}
impl<'a> PTC_W<'a> {
...
///Field `PSPD` reader - Port Speed: 00b: Low-speed 01b: Full-speed 10b: High-speed 11b: Reserved.
...
///Field `PSPD` writer - Port Speed: 00b: Low-speed 01b: Full-speed 10b: High-speed 11b: Reserved.
pub struct PSPD_W<'a> {
    w: &'a mut W,
}
impl<'a> PSPD_W<'a> {
...
///Field `WOO` reader - Wake on overcurrent enable: Writing this bit to a one enables the port to be sensitive to overcurrent conditions as wake-up events.
...
///Field `WOO` writer - Wake on overcurrent enable: Writing this bit to a one enables the port to be sensitive to overcurrent conditions as wake-up events.
pub struct WOO_W<'a> {
    w: &'a mut W,
}
impl<'a> WOO_W<'a> {
...
///Field `SUS_STAT` writer - These two bits are used by software to determine whether the most recent L1 suspend request was successful: 00b: Success-state transition was successful (ACK) 01b: Not Yet - Device was unable to enter the L1 state at this time (NYET) 10b: Not supported - Device does not support the L1 state (STALL) 11b: Timeout/Error - Device failed to respond or an error occurred.
...
///Field `DEV_ADD` reader - Device Address for LPM tokens.
...
///Field `DEV_ADD` writer - Device Address for LPM tokens.
