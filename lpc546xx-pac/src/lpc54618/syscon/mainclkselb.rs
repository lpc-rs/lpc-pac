...
#[repr(u8)]
pub enum SEL_A {
    ///0: MAINCLKSELA. Use the clock source selected in MAINCLKSELA register.
    MAINCLKSELA = 0,
    ///2: System PLL output (pll_clk)
    SYSTEM_PLL_OUTPUT = 2,
...
///Field `SEL` reader - Clock source for main clock source selector B. Selects the clock source for the main clock.
...
///Field `SEL` writer - Clock source for main clock source selector B. Selects the clock source for the main clock.
pub struct SEL_W<'a> {
    w: &'a mut W,
}
impl<'a> SEL_W<'a> {
...
///`read()` method returns [mainclkselb::R](R) reader structure
impl crate::Readable for MAINCLKSELB_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mainclkselb::W](W) writer structure
impl crate::Writable for MAINCLKSELB_SPEC {
    type Writer = W;
}
///`reset()` method sets MAINCLKSELB to value 0
