...
///Field `ENRL` reader - Enables the rising edge or level interrupt for each pin interrupt. Bit n configures the pin interrupt selected in PINTSELn. 0 = Disable rising edge or level interrupt. 1 = Enable rising edge or level interrupt.
...
///Field `ENRL` writer - Enables the rising edge or level interrupt for each pin interrupt. Bit n configures the pin interrupt selected in PINTSELn. 0 = Disable rising edge or level interrupt. 1 = Enable rising edge or level interrupt.
pub struct ENRL_W<'a> {
    w: &'a mut W,
}
impl<'a> ENRL_W<'a> {
...
///`read()` method returns [ienr::R](R) reader structure
impl crate::Readable for IENR_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [ienr::W](W) writer structure
impl crate::Writable for IENR_SPEC {
    type Writer = W;
}
///`reset()` method sets IENR to value 0
