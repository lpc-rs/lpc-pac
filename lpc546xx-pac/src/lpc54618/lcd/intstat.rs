...
///Field `FUFMIS` reader - FIFO underflow masked interrupt status.
...
///Field `LNBUMIS` reader - LCD next address base update masked interrupt status.
...
///Field `VCOMPMIS` reader - Vertical compare masked interrupt status.
...
///Field `BERMIS` reader - AHB master bus error masked interrupt status.
...
impl R {
    ///Bit 1 - FIFO underflow masked interrupt status.
    #[inline(always)]
    pub fn fufmis(&self) -> FUFMIS_R {
        FUFMIS_R::new(((self.bits >> 1) & 0x01) != 0)
    }
    ///Bit 2 - LCD next address base update masked interrupt status.
    #[inline(always)]
    pub fn lnbumis(&self) -> LNBUMIS_R {
        LNBUMIS_R::new(((self.bits >> 2) & 0x01) != 0)
    }
    ///Bit 3 - Vertical compare masked interrupt status.
    #[inline(always)]
    pub fn vcompmis(&self) -> VCOMPMIS_R {
        VCOMPMIS_R::new(((self.bits >> 3) & 0x01) != 0)
    }
    ///Bit 4 - AHB master bus error masked interrupt status.
    #[inline(always)]
