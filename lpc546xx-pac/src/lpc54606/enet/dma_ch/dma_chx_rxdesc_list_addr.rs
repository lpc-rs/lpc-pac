///Register `DMA_CHx_RXDESC_LIST_ADDR` reader
...
///Register `DMA_CHx_RXDESC_LIST_ADDR` writer
...
///Field `SRL` reader - Start of receive list This field contains the base address of the First in the Receive list.
...
///Field `SRL` writer - Start of receive list This field contains the base address of the First in the Receive list.
pub struct SRL_W<'a> {
    w: &'a mut W,
}
impl<'a> SRL_W<'a> {
...
///`read()` method returns [dma_chx_rxdesc_list_addr::R](R) reader structure
impl crate::Readable for DMA_CHX_RXDESC_LIST_ADDR_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dma_chx_rxdesc_list_addr::W](W) writer structure
impl crate::Writable for DMA_CHX_RXDESC_LIST_ADDR_SPEC {
    type Writer = W;
}
///`reset()` method sets DMA_CHx_RXDESC_LIST_ADDR to value 0
