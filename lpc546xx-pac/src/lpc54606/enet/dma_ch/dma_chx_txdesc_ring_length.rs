///Register `DMA_CHx_TXDESC_RING_LENGTH` reader
...
///Register `DMA_CHx_TXDESC_RING_LENGTH` writer
...
///Field `TDRL` reader - Transmit Ring Length This field sets the maximum number of Tx descriptors in the circular ring.
...
///Field `TDRL` writer - Transmit Ring Length This field sets the maximum number of Tx descriptors in the circular ring.
pub struct TDRL_W<'a> {
    w: &'a mut W,
}
impl<'a> TDRL_W<'a> {
...
///`read()` method returns [dma_chx_txdesc_ring_length::R](R) reader structure
impl crate::Readable for DMA_CHX_TXDESC_RING_LENGTH_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dma_chx_txdesc_ring_length::W](W) writer structure
impl crate::Writable for DMA_CHX_TXDESC_RING_LENGTH_SPEC {
    type Writer = W;
}
///`reset()` method sets DMA_CHx_TXDESC_RING_LENGTH to value 0
