...
///Field `RXQ_WEGT` reader - Receive Queue Weight This field indicates the weight assigned to the Rx Queue 0.
...
///Field `RXQ_WEGT` writer - Receive Queue Weight This field indicates the weight assigned to the Rx Queue 0.
...
///Field `RXQ_FRM_ARBIT` reader - Receive Queue Packet Arbitration When this bit is set, the The ethernet block drives the packet data to the ARI interface such that the entire packet data of currently-selected queue is transmitted before switching to other queue.
...
///Field `RXQ_FRM_ARBIT` writer - Receive Queue Packet Arbitration When this bit is set, the The ethernet block drives the packet data to the ARI interface such that the entire packet data of currently-selected queue is transmitted before switching to other queue.
...
///`read()` method returns [mtl_rxqx_ctrl::R](R) reader structure
impl crate::Readable for MTL_RXQX_CTRL_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mtl_rxqx_ctrl::W](W) writer structure
impl crate::Writable for MTL_RXQX_CTRL_SPEC {
    type Writer = W;
}
///`reset()` method sets MTL_RXQx_CTRL to value 0
