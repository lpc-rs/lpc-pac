...
///Field `RWCSTS` writer - MTL Rx Queue Write Controller Active Status When high, this bit indicates that the MTL Rx queue Write controller is active, and it is transferring a received packet to the Rx Queue.
...
///Field `RRCSTS` reader - MTL Rx Queue Read Controller State This field gives the state of the Rx queue Read controller: 00: Idle state 01: Reading packet data 10: Reading packet status (or timestamp) 11: Flushing the packet data and status.
...
///Field `RXQSTS` reader - MTL Rx Queue Fill-Level Status This field gives the status of the fill-level of the Rx Queue: 0x0: Rx Queue empty 0x1: Rx Queue fill-level below flow-control deactivate threshold 0x2: Rx Queue fill-level above flow-control activate threshold 0x3: Rx Queue full.
...
///Field `PRXQ` reader - Number of Packets in Receive Queue This field indicates the current number of packets in the Rx Queue.
...
///`read()` method returns [mtl_rxqx_dbg::R](R) reader structure
impl crate::Readable for MTL_RXQX_DBG_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mtl_rxqx_dbg::W](W) writer structure
impl crate::Writable for MTL_RXQX_DBG_SPEC {
    type Writer = W;
}
///`reset()` method sets MTL_RXQx_DBG to value 0
