...
///Field `A47_32` reader - MAC Address0 \[47:32\]
...
///Field `A47_32` writer - MAC Address0 \[47:32\]
///This field contains the upper 16 bits (47:32) of the 6-byte first MAC address.
...
///Field `DCS` reader - DMA Channel Select This field contains the DMA Channel number to which the Rx packet whose DA matches the MAC Address content is routed.
...
///Field `DCS` writer - DMA Channel Select This field contains the DMA Channel number to which the Rx packet whose DA matches the MAC Address content is routed.
pub struct DCS_W<'a> {
    w: &'a mut W,
}
impl<'a> DCS_W<'a> {
...
///`read()` method returns [mac_addr_high::R](R) reader structure
impl crate::Readable for MAC_ADDR_HIGH_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mac_addr_high::W](W) writer structure
impl crate::Writable for MAC_ADDR_HIGH_SPEC {
    type Writer = W;
}
///`reset()` method sets MAC_ADDR_HIGH to value 0x8000_ffff
