///Register `HCCONTROLCURRENTED` reader
...
///Register `HCCONTROLCURRENTED` writer
...
///Field `CCED` reader - ControlCurrentED.
...
///Field `CCED` writer - ControlCurrentED.
pub struct CCED_W<'a> {
    w: &'a mut W,
}
impl<'a> CCED_W<'a> {
...
///`read()` method returns [hccontrolcurrented::R](R) reader structure
impl crate::Readable for HCCONTROLCURRENTED_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [hccontrolcurrented::W](W) writer structure
impl crate::Writable for HCCONTROLCURRENTED_SPEC {
    type Writer = W;
}
///`reset()` method sets HCCONTROLCURRENTED to value 0
