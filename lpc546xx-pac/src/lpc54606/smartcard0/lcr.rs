...
///Field `WLS` reader - Word Length Select.
...
///Field `WLS` writer - Word Length Select.
pub struct WLS_W<'a> {
    w: &'a mut W,
}
impl<'a> WLS_W<'a> {
...
///Field `SBS` reader - Stop Bit Select.
...
///Field `SBS` writer - Stop Bit Select.
pub struct SBS_W<'a> {
    w: &'a mut W,
}
impl<'a> SBS_W<'a> {
