...
///Field `OFFSET` reader - Address bits 31:9 of the beginning of the DMA descriptor table. For 18 channels, the table must begin on a 512 byte boundary.
...
///Field `OFFSET` writer - Address bits 31:9 of the beginning of the DMA descriptor table. For 18 channels, the table must begin on a 512 byte boundary.
...
///`read()` method returns [srambase::R](R) reader structure
impl crate::Readable for SRAMBASE_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [srambase::W](W) writer structure
impl crate::Writable for SRAMBASE_SPEC {
    type Writer = W;
}
///`reset()` method sets SRAMBASE to value 0
