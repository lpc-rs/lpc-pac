...
///Field `ERR` reader - Error Interrupt flag for DMA channel n. Bit n corresponds to DMA channel n. The number of bits = number of DMA channels in this device. Other bits are reserved. 0 = error interrupt is not active. 1 = error interrupt is active.
...
///Field `ERR` writer - Error Interrupt flag for DMA channel n. Bit n corresponds to DMA channel n. The number of bits = number of DMA channels in this device. Other bits are reserved. 0 = error interrupt is not active. 1 = error interrupt is active.
pub struct ERR_W<'a> {
    w: &'a mut W,
}
impl<'a> ERR_W<'a> {
...
///`read()` method returns [errint0::R](R) reader structure
impl crate::Readable for ERRINT0_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [errint0::W](W) writer structure
impl crate::Writable for ERRINT0_SPEC {
    type Writer = W;
}
///`reset()` method sets ERRINT0 to value 0
