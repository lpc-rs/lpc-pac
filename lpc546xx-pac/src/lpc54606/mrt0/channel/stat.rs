...
///Field `INTFLAG` reader - Monitors the interrupt flag.
...
///Field `RUN` reader - Indicates the state of TIMERn. This bit is read-only.
...
///Field `INUSE` reader - Channel In Use flag. Operating details depend on the MULTITASK bit in the MODCFG register, and affects the use of IDLE_CH. See Idle channel register for details of the two operating modes.
...
///Field `INUSE` writer - Channel In Use flag. Operating details depend on the MULTITASK bit in the MODCFG register, and affects the use of IDLE_CH. See Idle channel register for details of the two operating modes.
