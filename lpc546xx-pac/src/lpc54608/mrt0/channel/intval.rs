...
///Field `IVALUE` reader - Time interval load value. This value is loaded into the TIMERn register and the MRT channel n starts counting down from IVALUE -1. If the timer is idle, writing a non-zero value to this bit field starts the timer immediately. If the timer is running, writing a zero to this bit field does the following: If LOAD = 1, the timer stops immediately. If LOAD = 0, the timer stops at the end of the time interval.
...
///Field `LOAD` reader - Determines how the timer interval value IVALUE -1 is loaded into the TIMERn register. This bit is write-only. Reading this bit always returns 0.
...
///`read()` method returns [intval::R](R) reader structure
impl crate::Readable for INTVAL_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [intval::W](W) writer structure
impl crate::Writable for INTVAL_SPEC {
    type Writer = W;
}
///`reset()` method sets INTVAL to value 0
