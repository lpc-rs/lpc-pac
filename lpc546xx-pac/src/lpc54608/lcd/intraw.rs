...
///Field `FUFRIS` reader - FIFO underflow raw interrupt status.
...
///Field `LNBURIS` reader - LCD next address base update raw interrupt status.
...
///Field `VCOMPRIS` reader - Vertical compare raw interrupt status.
...
///Field `BERRAW` reader - AHB master bus error raw interrupt status.
...
impl R {
    ///Bit 1 - FIFO underflow raw interrupt status.
    #[inline(always)]
    pub fn fufris(&self) -> FUFRIS_R {
        FUFRIS_R::new(((self.bits >> 1) & 0x01) != 0)
    }
    ///Bit 2 - LCD next address base update raw interrupt status.
    #[inline(always)]
    pub fn lnburis(&self) -> LNBURIS_R {
        LNBURIS_R::new(((self.bits >> 2) & 0x01) != 0)
    }
    ///Bit 3 - Vertical compare raw interrupt status.
    #[inline(always)]
    pub fn vcompris(&self) -> VCOMPRIS_R {
        VCOMPRIS_R::new(((self.bits >> 3) & 0x01) != 0)
    }
    ///Bit 4 - AHB master bus error raw interrupt status.
    #[inline(always)]
...
///`read()` method returns [intraw::R](R) reader structure
impl crate::Readable for INTRAW_SPEC {
    type Reader = R;
}
///`reset()` method sets INTRAW to value 0
