...
///Field `START` reader - Signature generation start address (corresponds to AHB byte address bits\[20:4\]).
...
///Field `START` writer - Signature generation start address (corresponds to AHB byte address bits\[20:4\]).
...
///`read()` method returns [fmsstart::R](R) reader structure
impl crate::Readable for FMSSTART_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [fmsstart::W](W) writer structure
impl crate::Writable for FMSSTART_SPEC {
    type Writer = W;
}
///`reset()` method sets FMSSTART to value 0
