...
///Field `IEN` reader - The SCT requests an interrupt when bit n of this register and the event flag register are both one (event 0 = bit 0, event 1 = bit 1, etc.). The number of bits = number of events in this SCT.
...
///Field `IEN` writer - The SCT requests an interrupt when bit n of this register and the event flag register are both one (event 0 = bit 0, event 1 = bit 1, etc.). The number of bits = number of events in this SCT.
pub struct IEN_W<'a> {
    w: &'a mut W,
}
impl<'a> IEN_W<'a> {
...
///`read()` method returns [even::R](R) reader structure
impl crate::Readable for EVEN_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [even::W](W) writer structure
