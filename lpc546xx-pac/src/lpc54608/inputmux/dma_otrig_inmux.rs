///Register `DMA_OTRIG_INMUX[%s]` reader
...
///Register `DMA_OTRIG_INMUX[%s]` writer
...
///Field `INP` reader - DMA trigger output number (decimal value) for DMA channel n (n = 0 to 19).
...
///Field `INP` writer - DMA trigger output number (decimal value) for DMA channel n (n = 0 to 19).
pub struct INP_W<'a> {
    w: &'a mut W,
}
impl<'a> INP_W<'a> {
...
///`read()` method returns [dma_otrig_inmux::R](R) reader structure
impl crate::Readable for DMA_OTRIG_INMUX_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dma_otrig_inmux::W](W) writer structure
impl crate::Writable for DMA_OTRIG_INMUX_SPEC {
    type Writer = W;
}
///`reset()` method sets DMA_OTRIG_INMUX[%s]
///to value 0x1f
