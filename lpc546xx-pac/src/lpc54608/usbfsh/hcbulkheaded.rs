...
///Field `BHED` reader - BulkHeadED HC traverses the bulk list starting with the HcBulkHeadED pointer.
...
///Field `BHED` writer - BulkHeadED HC traverses the bulk list starting with the HcBulkHeadED pointer.
pub struct BHED_W<'a> {
    w: &'a mut W,
}
impl<'a> BHED_W<'a> {
...
///`read()` method returns [hcbulkheaded::R](R) reader structure
impl crate::Readable for HCBULKHEADED_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [hcbulkheaded::W](W) writer structure
impl crate::Writable for HCBULKHEADED_SPEC {
    type Writer = W;
}
///`reset()` method sets HCBULKHEADED to value 0
