...
///Field `CLR` writer - Writing ones to this register clears the corresponding bits in ENABLESET0. Bit n clears the channel enable bit n. The number of bits = number of DMA channels in this device. Other bits are reserved.
pub struct CLR_W<'a> {
    w: &'a mut W,
}
impl<'a> CLR_W<'a> {
...
///`write(|w| ..)` method takes [enableclr0::W](W) writer structure
impl crate::Writable for ENABLECLR0_SPEC {
    type Writer = W;
}
///`reset()` method sets ENABLECLR0 to value 0
