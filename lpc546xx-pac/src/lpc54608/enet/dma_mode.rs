...
///Field `SWR` reader - Software Reset When this bit is set, the MAC and the OMA controller reset the logic and all internal registers of the OMA, MTL, and MAC.
...
///Field `SWR` writer - Software Reset When this bit is set, the MAC and the OMA controller reset the logic and all internal registers of the OMA, MTL, and MAC.
pub struct SWR_W<'a> {
    w: &'a mut W,
}
impl<'a> SWR_W<'a> {
...
///Field `DA` reader - DMA Tx or Rx Arbitration Scheme This bit specifies the arbitration scheme between the Transmit and Receive paths of all channels: The Tx path has priority over the Rx path when the TXPR bit is set.
...
///Field `DA` writer - DMA Tx or Rx Arbitration Scheme This bit specifies the arbitration scheme between the Transmit and Receive paths of all channels: The Tx path has priority over the Rx path when the TXPR bit is set.
pub struct DA_W<'a> {
    w: &'a mut W,
}
impl<'a> DA_W<'a> {
...
///Field `TAA` reader - Transmit Arbitration Algorithm This field is used to select the arbitration algorithm for the Transmit side when multiple Tx DMAs are selected.
...
///Field `TAA` writer - Transmit Arbitration Algorithm This field is used to select the arbitration algorithm for the Transmit side when multiple Tx DMAs are selected.
pub struct TAA_W<'a> {
    w: &'a mut W,
}
impl<'a> TAA_W<'a> {
...
///Field `PR` reader - Priority Ratio These bits control the priority ratio in weighted round-robin arbitration between the Rx DMA and Tx DMA.
...
///Field `PR` writer - Priority Ratio These bits control the priority ratio in weighted round-robin arbitration between the Rx DMA and Tx DMA.
pub struct PR_W<'a> {
    w: &'a mut W,
}
impl<'a> PR_W<'a> {
...
///`read()` method returns [dma_mode::R](R) reader structure
impl crate::Readable for DMA_MODE_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dma_mode::W](W) writer structure
impl crate::Writable for DMA_MODE_SPEC {
    type Writer = W;
}
///`reset()` method sets DMA_MODE to value 0
