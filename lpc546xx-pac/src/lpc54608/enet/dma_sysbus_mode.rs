///Register `DMA_SYSBUS_MODE` reader
...
///Register `DMA_SYSBUS_MODE` writer
...
///Field `FB` reader - Fixed Burst Length When this bit is set to 1, the AHB master will initiate burst transfers of specified length (INCRx or SINGLE).
...
///Field `FB` writer - Fixed Burst Length When this bit is set to 1, the AHB master will initiate burst transfers of specified length (INCRx or SINGLE).
pub struct FB_W<'a> {
    w: &'a mut W,
}
impl<'a> FB_W<'a> {
...
///Field `AAL` reader - Address-Aligned Beats When this bit is set to 1, the AHB master performs address-aligned burst transfers on Read and Write channels.
...
///Field `AAL` writer - Address-Aligned Beats When this bit is set to 1, the AHB master performs address-aligned burst transfers on Read and Write channels.
pub struct AAL_W<'a> {
    w: &'a mut W,
}
impl<'a> AAL_W<'a> {
...
///Field `MB` reader - Mixed Burst When this bit is set high and the FB bit is low, the AHB master performs undefined bursts transfers (INCR) for burst length of 16 or more.
...
///Field `MB` writer - Mixed Burst When this bit is set high and the FB bit is low, the AHB master performs undefined bursts transfers (INCR) for burst length of 16 or more.
pub struct MB_W<'a> {
    w: &'a mut W,
}
impl<'a> MB_W<'a> {
...
///Field `RB` reader - Rebuild INCRx Burst When this bit is set high and the AHB master gets SPLIT, RETRY, or EarlyBurst Termination (EBT) response, the AHB master interface rebuilds the pending beats of any initiated burst transfer with INCRx and SINGLEtransfers.
...
///Field `RB` writer - Rebuild INCRx Burst When this bit is set high and the AHB master gets SPLIT, RETRY, or EarlyBurst Termination (EBT) response, the AHB master interface rebuilds the pending beats of any initiated burst transfer with INCRx and SINGLEtransfers.
pub struct RB_W<'a> {
    w: &'a mut W,
}
impl<'a> RB_W<'a> {
...
///`read()` method returns [dma_sysbus_mode::R](R) reader structure
impl crate::Readable for DMA_SYSBUS_MODE_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dma_sysbus_mode::W](W) writer structure
impl crate::Writable for DMA_SYSBUS_MODE_SPEC {
    type Writer = W;
}
///`reset()` method sets DMA_SYSBUS_MODE to value 0
