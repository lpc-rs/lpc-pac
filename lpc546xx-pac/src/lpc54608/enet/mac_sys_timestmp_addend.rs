///Register `MAC_SYS_TIMESTMP_ADDEND` reader
...
///Register `MAC_SYS_TIMESTMP_ADDEND` writer
...
///Field `TSAR` reader - Time stamp addend This register indicates the 32-bit time value to be added to the Accumulator register to achieve time synchronization.
...
///Field `TSAR` writer - Time stamp addend This register indicates the 32-bit time value to be added to the Accumulator register to achieve time synchronization.
pub struct TSAR_W<'a> {
    w: &'a mut W,
}
impl<'a> TSAR_W<'a> {
...
///`read()` method returns [mac_sys_timestmp_addend::R](R) reader structure
impl crate::Readable for MAC_SYS_TIMESTMP_ADDEND_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [mac_sys_timestmp_addend::W](W) writer structure
impl crate::Writable for MAC_SYS_TIMESTMP_ADDEND_SPEC {
    type Writer = W;
}
///`reset()` method sets MAC_SYS_TIMESTMP_ADDEND to value 0
