...
///Field `TXIDLE` reader - Transmitter Idle status.
...
///Field `DELTACTS` reader - This bit is set when a change in the state of the CTS input is detected.
...
///Field `TXDISINT` reader - Transmitter Disabled Interrupt flag.
...
///Field `DELTARXBRK` reader - This bit is set when a change in the state of receiver break detection occurs.
...
///Field `START` reader - This bit is set when a start is detected on the receiver input.
...
///Field `FRAMERRINT` reader - Framing Error interrupt flag.
...
///Field `PARITYERRINT` reader - Parity Error interrupt flag.
...
///Field `RXNOISEINT` reader - Received Noise interrupt flag.
...
///Field `ABERRINT` reader - Auto baud Error Interrupt flag.
