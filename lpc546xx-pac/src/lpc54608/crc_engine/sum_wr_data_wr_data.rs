...
///Field `CRC_WR_DATA` writer - Data written to this register will be taken to perform CRC calculation with selected bit order and 1's complement pre-process. Any write size 8, 16 or 32-bit are allowed and accept back-to-back transactions.
...
///`write(|w| ..)` method takes [sum_wr_data_wr_data::W](W) writer structure
impl crate::Writable for SUM_WR_DATA_WR_DATA_SPEC {
    type Writer = W;
}
///`reset()` method sets WR_DATA to value 0
