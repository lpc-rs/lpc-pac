...
impl From<crate::R<GAINSHIFT_SPEC>> for R {
    #[inline(always)]
    fn from(reader: crate::R<GAINSHIFT_SPEC>) -> Self {
        R(reader)
    }
}
///Register `GAINSHIFT` writer
...
impl From<crate::W<GAINSHIFT_SPEC>> for W {
    #[inline(always)]
    fn from(writer: crate::W<GAINSHIFT_SPEC>) -> Self {
        W(writer)
    }
}
///Field `GAIN` reader - Gain control, as a positive or negative (two's complement) number of bits to shift.
...
///Field `GAIN` writer - Gain control, as a positive or negative (two's complement) number of bits to shift.
pub struct GAIN_W<'a> {
    w: &'a mut W,
}
impl<'a> GAIN_W<'a> {
...
///`read()` method returns [gainshift::R](R) reader structure
impl crate::Readable for GAINSHIFT_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [gainshift::W](W) writer structure
impl crate::Writable for GAINSHIFT_SPEC {
    type Writer = W;
}
///`reset()` method sets GAINSHIFT to value 0
