...
///Field `CAP_VALUE` reader - Capture value for the related capture event (UTICK_CAPn. Note: the value is 1 lower than the actual value of the Micro-tick Timer at the moment of the capture event.
...
///Field `VALID` reader - Capture Valid. When 1, a value has been captured based on a transition of the related UTICK_CAPn pin. Cleared by writing to the related bit in the CAPCLR register.
...
///`read()` method returns [cap::R](R) reader structure
impl crate::Readable for CAP_SPEC {
    type Reader = R;
}
///`reset()` method sets CAP[%s]
///to value 0
