...
///Field `RAS` reader - RAS latency (active to read/write delay).
...
///Field `RAS` writer - RAS latency (active to read/write delay).
pub struct RAS_W<'a> {
    w: &'a mut W,
}
impl<'a> RAS_W<'a> {
...
///`read()` method returns [dynamicrascas::R](R) reader structure
impl crate::Readable for DYNAMICRASCAS_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [dynamicrascas::W](W) writer structure
impl crate::Writable for DYNAMICRASCAS_SPEC {
    type Writer = W;
}
///`reset()` method sets DYNAMICRASCAS to value 0x0303
