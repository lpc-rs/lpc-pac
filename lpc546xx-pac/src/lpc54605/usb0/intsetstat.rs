...
///Field `EP_SET_INT` reader - If software writes a one to one of these bits, the corresponding USB interrupt status bit is set. When this register is read, the same value as the USB interrupt status register is returned.
...
///Field `EP_SET_INT` writer - If software writes a one to one of these bits, the corresponding USB interrupt status bit is set. When this register is read, the same value as the USB interrupt status register is returned.
...
///Field `FRAME_SET_INT` reader - If software writes a one to one of these bits, the corresponding USB interrupt status bit is set. When this register is read, the same value as the USB interrupt status register is returned.
...
///Field `FRAME_SET_INT` writer - If software writes a one to one of these bits, the corresponding USB interrupt status bit is set. When this register is read, the same value as the USB interrupt status register is returned.
...
///Field `DEV_SET_INT` writer - If software writes a one to one of these bits, the corresponding USB interrupt status bit is set. When this register is read, the same value as the USB interrupt status register is returned.
...
///`read()` method returns [intsetstat::R](R) reader structure
impl crate::Readable for INTSETSTAT_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [intsetstat::W](W) writer structure
impl crate::Writable for INTSETSTAT_SPEC {
    type Writer = W;
}
///`reset()` method sets INTSETSTAT to value 0
