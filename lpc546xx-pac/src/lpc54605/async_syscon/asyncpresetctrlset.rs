///Register `ASYNCPRESETCTRLSET` writer
...
///Field `ARST_SET` writer - Writing ones to this register sets the corresponding bit or bits in the ASYNCPRESETCTRL register, if they are implemented. Bits that do not correspond to defined bits in ASYNCPRESETCTRL are reserved and only zeroes should be written to them.
...
///`write(|w| ..)` method takes [asyncpresetctrlset::W](W) writer structure
impl crate::Writable for ASYNCPRESETCTRLSET_SPEC {
    type Writer = W;
}
///`reset()` method sets ASYNCPRESETCTRLSET to value 0
