///Register `HCPERIODICSTART` reader
...
///Register `HCPERIODICSTART` writer
...
///Field `PS` reader - PeriodicStart After a hardware reset, this field is cleared and then set by HCD during the HC initialization.
...
///Field `PS` writer - PeriodicStart After a hardware reset, this field is cleared and then set by HCD during the HC initialization.
pub struct PS_W<'a> {
    w: &'a mut W,
}
impl<'a> PS_W<'a> {
...
///`read()` method returns [hcperiodicstart::R](R) reader structure
impl crate::Readable for HCPERIODICSTART_SPEC {
    type Reader = R;
}
///`write(|w| ..)` method takes [hcperiodicstart::W](W) writer structure
impl crate::Writable for HCPERIODICSTART_SPEC {
    type Writer = W;
}
///`reset()` method sets HCPERIODICSTART to value 0
