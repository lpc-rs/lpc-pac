#[doc = "Register `DMA_CHx_RX_INT_WDTIMER` reader"]
...
impl core::convert::From<crate::R<DMA_CHX_RX_INT_WDTIMER_SPEC>> for R {
    fn from(reader: crate::R<DMA_CHX_RX_INT_WDTIMER_SPEC>) -> Self {
        R(reader)
    }
}
#[doc = "Register `DMA_CHx_RX_INT_WDTIMER` writer"]
...
impl core::convert::From<crate::W<DMA_CHX_RX_INT_WDTIMER_SPEC>> for W {
    fn from(writer: crate::W<DMA_CHX_RX_INT_WDTIMER_SPEC>) -> Self {
        W(writer)
    }
}
#[doc = "Field `RIWT` reader - Receive Interrupt Watchdog Timer Count Indicates the number of system clock cycles multiplied by 256 for which the watchdog timer is set."]
...
#[doc = "Field `RIWT` writer - Receive Interrupt Watchdog Timer Count Indicates the number of system clock cycles multiplied by 256 for which the watchdog timer is set."]
pub struct RIWT_W<'a> {
    w: &'a mut W,
}
impl<'a> RIWT_W<'a> {
...
#[doc = "`read()` method returns [dma_chx_rx_int_wdtimer::R](R) reader structure"]
impl crate::Readable for DMA_CHX_RX_INT_WDTIMER_SPEC {
    type Reader = R;
}
#[doc = "`write(|w| ..)` method takes [dma_chx_rx_int_wdtimer::W](W) writer structure"]
impl crate::Writable for DMA_CHX_RX_INT_WDTIMER_SPEC {
    type Writer = W;
}
#[doc = "`reset()` method sets DMA_CHx_RX_INT_WDTIMER to value 0"]
