...
impl core::convert::From<crate::R<DMA_INMUX_INMUX_SPEC>> for R {
...
impl core::convert::From<crate::W<DMA_INMUX_INMUX_SPEC>> for W {
    fn from(writer: crate::W<DMA_INMUX_INMUX_SPEC>) -> Self {
        W(writer)
    }
}
#[doc = "Field `INP` reader - DMA trigger output number (decimal value) for DMA channel n (n = 0 to 17)."]
...
#[doc = "Field `INP` writer - DMA trigger output number (decimal value) for DMA channel n (n = 0 to 17)."]
pub struct INP_W<'a> {
    w: &'a mut W,
}
impl<'a> INP_W<'a> {
